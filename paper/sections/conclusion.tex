...
Paper~I's bipartite-plaquette Q-tensor~\cite{wilson1974confinement}
(eigenvalues $\{4, 4, 16\}$) is inherited unchanged for
non-abelian gauge groups; the SU(N) trace identity introduces
only a global $T_F = 1/2$ factor.  Spectator-factor counting on
the per-site $\mathbb{C}^{12}$ amplitude assigns each gauge factor
the product of the dimensions of the tensor slots it does \emph{not}
act on:
\begin{align}
N_f^{SU(2)_W} &= \dim(\mathbb{C}^2_\text{chir}) \cdot \dim(\mathbb{C}^3_\text{col}) = 2 \cdot 3 = 6, \nonumber\\
N_f^{SU(3)_c} &= \dim(\mathbb{C}^2_\text{chir}) \cdot \dim(\mathbb{C}^2_\text{iso}) = 2 \cdot 2 = 4, \nonumber\\
N_f^{U(1)} &= \dim(\mathbb{C}^{12}) = 12. \nonumber
\end{align}
Multiplying by $T_F = 1/2$ for SU(N) and unit charge-squared for
U(1) gives the framework's quantitative gauge-coupling prediction:
\begin{equation}
...
\begin{equation}
\label{eq:g3_over_g2}
g_3^2 / g_2^2 \;=\; \frac{N_f^{SU(2)_W} \cdot T_F}{N_f^{SU(3)_c} \cdot T_F} \;=\; \frac{6 \cdot (1/2)}{4 \cdot (1/2)} \;=\; \frac{3}{2} \quad \text{at the lattice scale,}
\end{equation}
...
\begin{figure}[H]
    \centering
    \input{figures/centralizer_block_diagram}
\end{figure}
...
(Fig.~\ref{fig:centralizer_block_diagram}).
...
The parallel to Pati-Salam $SU(4)_C \times SU(2)_L \times SU(2)_R$
and its extensions is structural.  Pati-Salam postulates a
left-right symmetric gauge group at high energy with a
$\mathbb{Z}_2$ left-right exchange; the SM emerges by spontaneous
breaking, with the broken generators acquiring mass.  The
framework's $\mathfrak{su}(6) \oplus \mathfrak{su}(6) \oplus
\mathfrak{u}(1)$ shares this shape but with different content
(each $\mathfrak{su}(6)$ acts on $\mathbb{C}^2_\text{iso} \otimes
\mathbb{C}^3_\text{col} = \mathbb{C}^6$ rather than on
Pati-Salam's $SU(4)_C$ lepton-quark unification), and -- crucially
-- it is \emph{not postulated} but rather \emph{forced} by the
discrete bipartite geometry plus $\mathcal{A}=1$.  The fact that
left-right symmetric grand-unification-style structure emerges
from the substrate alone, without any gauge-group ansatz,
suggests the lattice is the right object for this kind of
question.  Whether the framework's specific $\mathfrak{su}(6)
\oplus \mathfrak{su}(6) \oplus \mathfrak{u}(1)$ admits a
phenomenological symmetry-breaking story (the 59 extras getting
large masses, the SM emerging at observable energies) is a
follow-on question this paper does not settle.

\item The SM gauge group $SU(3) \times SU(2) \times U(1)$ is the
\emph{factor-product projection} of this larger structure: the
sub-algebra obtained by restricting each Hermitian generator to
its action on a single tensor factor of
$\mathbb{C}^{12} = \mathbb{C}^2_\text{chir} \otimes
\mathbb{C}^2_\text{iso} \otimes \mathbb{C}^3_\text{col}$ (or
overall phase), setting the other factors to the identity.
Equality $=$ in Eq.~\eqref{eq:automorphism_conjecture} is
recovered when this factor-product gauge invariance is imposed
as an additional constraint beyond commutativity with the
bipartite tick rule (sufficiency established; necessity --- that
factor-product invariance is the unique additional constraint that
recovers equality --- is not independently verified).  The
framework's lattice geometry admits the larger algebra naturally
and the projection is an external interpretive restriction, not a
structural feature of the substrate.  The 59
``extras'' (Eq.~\eqref{eq:extras_decomp}) are the coset generators
sent to zero by the projection; they transform as
$(\mathbf{8}, \mathbf{3})$ leptoquark-flavoured plus
chirality-shadow SM under the SM adjoint action.  The relevant
$\mathfrak{su}(N)$ branching is standard
representation theory~\cite{georgi1999lie}.
...
not the prose.  In the present paper, the convention prevented over-claiming at
every phase boundary: Phase~1's chirality-orthogonal Bloch outcome
appears in the table as the chirality-alignment \texttt{PART} row
with the orthogonality result as its evidence, rather than as a
\texttt{PASS} on alignment; Phase~3's finding that the
discrete-Hermitian centralizer is 71-dim sits in the
``equality vs containment'' \texttt{PART} row, with the
$\mathfrak{su}(6) \oplus \mathfrak{su}(6) \oplus \mathfrak{u}(1)$
structure in its mechanism column, rather than being rephrased
into a $\texttt{PASS}$ that does not hold; Phase~1.5's
ruling-out of the antilinear $CP$ extension is recorded as the
``modified tick rule for Branch~B'' \texttt{FAIL} row -- a
publishable structural finding rather than a deleted negative
result.  The convention is a small mechanical change with a
...
the explicit $-\mathrm{Tr}\,\ln D_\text{lat}[U]$ calculation
(the Sakharov-Zeldovich induced-action
programme~\cite{sakharov1968vacuum} applied to the bipartite
Dirac operator) as a ``paper-length follow-on''; Paper~II
inherits this open item.  Closing it would fix the absolute scale
of $1/g^2(1/a)$, enabling a quantitative RG-running check against
the measured $g_i^2(M_Z)$ values.
...
\paragraph{Roadmap to chirality.}  Phase~1's orthogonal-Bloch
characterisation isolates the chirality question precisely.  The
lattice has a bipartite $\mathbb{Z}_2$ generated by $\sigma_x
\otimes I_2 \otimes I_3$ (spatial parity, swaps $L \leftrightarrow
R$); the SM has a chirality $\mathbb{Z}_2$ generated by $\gamma_5
= \sigma_z \otimes I_2 \otimes I_3$ (chirality projector,
distinguishes $L$ from $R$).  These two involutions anticommute
on the chirality $\mathbb{C}^2$ --- maximally orthogonal Bloch
axes.  The follow-on question is sharp: \emph{what structural
enrichment of the lattice (per-site Hilbert space, tick-rule
modification, additional discrete symmetry) makes both
involutions simultaneously available, with the SM's chirality
becoming an emergent feature of the enriched substrate rather
than an external choice?}  The Phase~1.5 ruling against
antilinear modifications shows that one obvious route is closed;
the open route is the structural enrichment direction.  A
plausible next paper would investigate whether the per-site
amplitude admits a chirality-twin extension
$\mathbb{C}^{12} \to \mathbb{C}^{12} \oplus \mathbb{C}^{12}$ that
makes the two $\mathbb{Z}_2$s simultaneously linear, with the
framework's $\mathcal{A}=1$ accounting tracking the relationship
between the two blocks.  This question is left precisely posed
by the present paper but not answered.
