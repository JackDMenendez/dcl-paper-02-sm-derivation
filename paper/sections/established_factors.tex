...
\eqref{eq:automorphism_conjecture}.  Equality $=$ is recovered
when the SM's tensor-factor-product gauge invariance is imposed
as an additional constraint --- specifically, the
\emph{factor-product projection}, the map that restricts each
generator to its action on a single tensor factor of
$\mathbb{C}^{12}$ (chirality, isospin, or colour), setting the
other factors to the identity.  (Sufficiency: the factor-product
projection lands on the 18-dim subalgebra of
Eq.~\eqref{eq:automorphism_conjecture}.  Necessity --- whether
factor-product invariance is the unique additional constraint that
recovers equality --- is not independently verified in this paper
and is left as an open question.)  Without this restriction, the
lattice's discrete-Hermitian centralizer admits factor-mixing
operators that the SM gauge structure forbids by construction;
Section~\ref{sec:conclusion} states the explicit
$\mathfrak{su}(6) \oplus \mathfrak{su}(6) \oplus \mathfrak{u}(1)$
structure of the centralizer and decomposes the factor-mixing
extras under the SM adjoint action.
