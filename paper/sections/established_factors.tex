...
% Structural prose for the six PASS rows of audit_table.tex.  Each
...
% six rows into the statement "established containment $\supseteq$"
...
The audit table (Table~\ref{tab:audit}) records six PASS rows of
Eq.~\eqref{eq:automorphism_conjecture}.  Each is a sympy-verified
statement on the bipartite octahedral lattice
$\mathcal{T}_\diamond^3$.  This section explains what the six rows
...
The six PASS rows combine as follows.
...
\item \emph{Chirality coupling alignment}: whether the framework's
bipartite parity (the RGB/CMY $\mathbb{Z}_2$) is the SM's chirality
projector, and whether the proposed $SU(2)_W$ on the isospin
$\mathbb{C}^2$ couples to $\psi_R$ and $\psi_L$ asymmetrically as
the Standard Model demands.  The trivial tensor extension
\eqref{eq:trivial_tensor_extension} is symmetric in $\psi_R$ vs
$\psi_L$; the Standard Model is not.
