...
Discrete spatial $\mathrm{Aut}(\Gamma, \mathbf{V})$ has order 48
    ...
    & \texttt{automorphism\_discrete.py} (sympy)
    & \texttt{PASS} \\
...
RGB sublattice symmetry contributes only $\mathbb{Z}_3 \subset SU(3)$
    ...
    & \texttt{automorphism\_rgb\_su3.py} (sympy)
    & \texttt{PASS} \\
...
$SO(3,1) \times U(1)$ acts on the existing $\mathbb{C}^2 = (\psi_R, \psi_L)$
    ...
    & Real dim $6 + 1 = 7$, direct product on the existing
      per-site amplitude
    & \texttt{automorphism\_direct\_product.py} (sympy);
...
Direct-product structure on extended $\mathbb{C}^{12}$
    ...
    & \texttt{automorphism\_direct\_product\_extended.py} (sympy)
    & \texttt{PASS} \\
...
Tick-rule consistency on $\mathbb{C}^{12}$
    ...
    & \texttt{tick\_rule\_extended\_consistency.py} (sympy);
      four checks pass
    & \texttt{PASS} \\
...
Wilson plaquette gauge invariance
    ...
    & \texttt{tick\_rule\_gauge\_invariance.py} (sympy);
...
Exact equality vs containment in $\mathrm{Aut}_\text{ext}$
    & Established: $\supseteq$ (the four conjecture factors).
      Conjectured: $=$ (no additional symmetries beyond the
      four-factor product)
    & Constrained by commutation with the established factors;
      tractable by Lie-algebra enumeration on the extended
      generators
    & Open follow-on calculation;
      strategy in
      \nolinkurl{notes/lie_algebra_proof_sketch.md}
    & \texttt{STUB} \\

SM-chirality coupling alignment
    & (a) Whether bipartite RGB/CMY parity is the same
      $\mathbb{Z}_2$ as the SM's chirality projector;
      (b) whether the proposed $SU(2)_W$ couples asymmetrically
      to $\psi_R$ vs $\psi_L$ as required by the SM
    & SM left-handed doublets / right-handed singlets;
      empirically required, no fitting freedom
    & Open structural-alignment problem; the load-bearing
      question that decides whether Paper~II derives the SM
      gauge group or characterises the obstruction
    & \texttt{STUB} \\

Explicit $1/g^2$ prefactor for $SU(2)_W$, $SU(3)$ Wilson actions
    & Generalisation of the Wilson-action template
      $S_W = (1/g^2)\sum_p (1 - \tfrac{1}{N}\mathrm{Re}\,\mathrm{Tr}\,W_p)$
      to non-abelian link variables
    & Continuum-limit identification with SM gauge-boson
      dynamics; the Sakharov-style induced-action calculation
      (Paper~I Appendix B) generalises with matrix-valued links
    & Open follow-on calculation;
      Paper~I's induced gauge-action appendix supplies the $U(1)$
      template
    & \texttt{STUB} \\
