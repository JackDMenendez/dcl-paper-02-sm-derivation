...
Exact equality vs containment in $\mathrm{Aut}_\text{ext}$
    & The discrete-Hermitian centralizer of the bipartite tick
      rule on $\mathbb{C}^{12}$ is $\mathfrak{Aut}_\text{cent} =
      \mathfrak{su}(6)_+ \oplus \mathfrak{su}(6)_- \oplus
      \mathfrak{u}(1)$ of dim 71, with the two $\mathfrak{su}(6)$
      factors acting on the $\pm 1$ chirality eigenspaces and the
      $\mathfrak{u}(1)$ central (= $J_1$).  Of the 18 Lie
      generators in Eq.~(137), 14 are Hermitian (3 rotations $J_a$,
      3 SU(2)$_W$, 8 SU(3)$_c$) and 4 are anti-Hermitian (the 3
      Lorentz boosts $K_a$ and the U(1) overall phase $Q = iI$).
      Of the 14 Hermitian generators, exactly 12 lie in the
      Hermitian centralizer ($J_1$, 3 $SU(2)_W$, 8 $SU(3)_c$);
      $J_2, J_3$ are continuum-emergent (they do not commute with
      the discrete $\sigma_x \otimes I_2 \otimes I_3$ but emerge
      from the $O_h$-averaging argument of Paper~I~§6,
      \cite{menendez2026geometry})
    & The 59 extra generators decompose under
      $SU(3) \times SU(2)$ as 48 of $(\mathbf{8}, \mathbf{3})$
      (leptoquark-flavoured, factor-mixing) plus 11
      $\sigma_x$-twisted SM-flavoured.  Identical structural
      pattern to left-right-symmetric grand unification (Pati-Salam,
      $SO(10)$) emerging from the bipartite lattice
      geometry alone.  Equality $=$ in Eq.~(137) holds iff
      tensor-factor-product gauge invariance is imposed as an
    ...
    & \nolinkurl{automorphism_centralizer_extended.py} (sympy);
      ...
      Bracket structure verified by
      \nolinkurl{aut_centralizer_extras_commutators.py} (sympy):
      $[\text{extras}, \text{SM}] \subseteq \text{extras}$
      (708 brackets, 0 in SM, 452 in extras);
      $[\text{extras}, \text{extras}]$ has 226/1711 brackets in SM
      (extras is SM-module, not Lie ideal --- broken-symmetry shape)
    & \texttt{PART} \\
...
\hline
% ── v2.0: the single-domain no-go (Paper IV) ─────────────────────────
% Mirrors the row added to Paper I's audit table at its v2.0.
Single-domain gauge viability
    & Three-diagonal hop set $\{V_1, V_2, V_3\}$, one domain
      (omits the fourth cube body diagonal $V_4 = (1,1,-1)$).
      This is the substrate on which every row above is computed
    & Common-mode photon-speed anisotropy is $O(1)$ at mass
      dimension four, not suppressed; matter sector likewise
      excluded as constructed.  The optical axis is observable
      but is not observed, so the geometry must change; the
      successor architecture is not birefringent.
      $\mathcal{A}=1$ is not falsified by this result
    & Paper~IV~\cite{menendez2026opticalaxis}: referee M1/M2,
      verified \nolinkurl{exp_05_matter_sector_order};
      excluded by cavity / Michelson--Morley isotropy bounds.
      See Paper~IV for the full account
    & \texttt{FAIL} \\
...
$\mathrm{Aut}(\mathcal{T}_\diamond^3, \mathcal{A}=1) = SO(3,1) \times SU(3) \times SU(2) \times U(1)$
    & Containment $\supseteq$ established (18-dim direct sum
      verified as a subalgebra of the lattice's centralizer);
      strict equality $=$ does \emph{not} hold at the discrete
      level (the centralizer is $\mathfrak{su}(6) \oplus
      \mathfrak{su}(6) \oplus \mathfrak{u}(1)$, dim 71, with the
      SM 12-dim algebra as its factor-product-irreducible
      subalgebra).  The SM gauge group is recovered as the
      \emph{factor-product projection} of the lattice's
      $\mathcal{A}=1$ symmetry algebra, not as its full
      automorphism group
    & Standard Model gauge group plus Lorentz, derived from a
      single conservation axiom on a discrete substrate as a
      \emph{characterisation}: the SM emerges as the
      factor-product-effective subalgebra of a larger
      lattice-level symmetry, with the 59 ``extras'' carrying a
      specific representation content $(\mathbf{8}, \mathbf{3})$
      under $SU(3) \times SU(2)$ that gives the framework its
      quantitative prediction $g_3^2/g_2^2 = 3/2$ at the lattice
      scale.  Chirality and $CP$ structure NOT recovered (Phase~1
      Route~(a) and Phase~1.5 Route~(b))
    & Conjunction of all sub-claims above: 8 PASS (containment +
      structural foundations) + 3 PART (chirality alignment,
      equality vs containment, $1/g^2$ ratio) + 2 FAIL (SM-$CP$
      from antilinear modification; single-domain gauge
      viability).  Notes:
