...
\begin{tikzpicture}[
    ...
    box/.style={
        ...
        align=center, inner sep=5pt, minimum height=2.4em,
    },
    smbox/.style={
...
\node[topbox, text width=8.0cm] (cone) at (0, 7.6) {%
    per-site amplitude \\[2pt]
...
\node[smbox, text width=3.6cm] (sm) at (-2.9, 2.0) {%
    SM subalgebra \\
    $\mathfrak{su}(3)_c \oplus \mathfrak{su}(2)_W \oplus \langle J_1 \rangle$ \\
    \scriptsize dim 12
};
\node[extbox, text width=4.8cm] (ext) at (2.75, 2.0) {%
    extras (SM-module, not Lie ideal) \\
    $48 \times (\mathbf{8}, \mathbf{3})_\text{leptoquark}$ \\
    $\;+\; 11 \times \sigma_x\text{-shadow SM}$ \\
    \scriptsize dim 59
};
\node[centbox, fit={(sm) (ext)}, inner sep=10pt, fill=none] (cent) {};
\node[anchor=south, font=\small, yshift=2pt] at (cent.north) {%
    centralizer \;
...
\draw[armow] (cone.south) -- node[right, label, pos=0.45] {%
    Hermitian operators commuting \\
    with $\sigma_x \otimes I_2 \otimes I_3$
} (cent.north -| cone);

% Layer 3: the SM gauge algebra + Lorentz
\node[smbox, minimum width=8.5cm, text width=8.0cm] (eq137) at (0, -1.6) {%
    Eq.~(137) RHS:\quad
